...
import com.CustomExceptions.InvaliPinException;
...
public class ATMOperations {
	...
	public static IATMService validateCredentials(long cardNumber, int pinNumber)
			throws InvalidCardException, InvaliPinException, IncorrectPinLimitReachedException {
		if (dataBase.containsKey(cardNumber)) {
		...
		try {
			if (card.getUserType().equals("operator")) {
				...
				if (card.getPinNumber() != pinNumber) {
					throw new InvaliPinException("Dear operator, Please enter correct PIN Number");
				} else {
					return card;
				}
			}
		...
		if (card.getPinNumber() != pinNumber) {
			...
			throw new InvaliPinException("You have entered a wrong Pin Number");
		} else {
			return card;
		}
	}
	...
	public static void main(String[] args) throws NotAOperatorException {
		...
		dataBase.put(22222222222l, new AxisDebitCard(22222222222l, "Pavan", 50000.0, 2222));
		dataBase.put(333333333333l, new HDFCDebitCard(333333333333l, "Naveen", 70000.0, 3333));
		dataBase.put(44444444444l, new SBIDebitCard(44444444444l, "Pa1", 85000.0, 4444));
		...
		while (MACHINE_ON) {
			...
			} catch (InvaliPinException | InvalidCardException | IncorrectPinLimitReachedException e) {
